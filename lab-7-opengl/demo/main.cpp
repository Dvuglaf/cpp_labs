...
const size_t N = 200;                                 // Количество точек
...
int main() {
    try {
        ...
        GLFWwindow* const window = glfwCreateWindow(640, 640, "Demo", nullptr, nullptr);;
        if (window == nullptr)
      ...
      //  const auto initMinSpeedLocation = glGetUniformLocation(initProgram.id, "minSpeed");
       // const auto initMaxSpeedLocation = glGetUniformLocation(initProgram.id, "maxSpeed");
        ...
        GLuint vbo;
        {
            ...
            glBufferData(GL_ARRAY_BUFFER, N * STRIDE, data.data(), GL_STREAM_READ);
        }
