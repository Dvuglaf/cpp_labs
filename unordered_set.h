...
public:
	...
	void insert(const T& obj) {
		...
		if (table[index]->next == nullptr) {//если нет коллизии
		...
		else if (table[index]->next != nullptr)
		{
			...
			while (ptr->next != nullptr) {//до конца списка
				ptr = ptr->next;
			}
			ptr->next = element;
			element->data = obj;
	...
	void erase(const T& key) {
		...
		auto ptr = table[index];
		if (ptr->next != nullptr) {
			while (ptr->next->data != key) {//получаем указатель на элемент, стоящий до удаляемого
				ptr = ptr->next;
				if (ptr->next == nullptr)
					return;
			}
	...
	u_node<T>* find(const T& key) const {
		...
		auto ptr = table[index];
		while (ptr->next != nullptr && ptr->next->data != key) {//до конца, либо до key
			ptr = ptr->next;
		}
